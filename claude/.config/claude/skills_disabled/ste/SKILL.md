...
| `references/word-index.tsv` | 2196 rows. 879 approved | no |
...
## If a reference file is absent

Build it yourself. Do not ask the user to do these steps, and do not state a
rule or a word from memory.

1. If `references/ASD-STE100_ISSUE9.pdf` is absent, download it:

   ```
   curl -sSL --max-time 120 -o references/ASD-STE100_ISSUE9.pdf \
     https://www.asd-ste100.org/assets/files/ASD-STE100_ISSUE9.pdf
   ```

   ASD hosts Issue 9 at that address, with no request form. The correct file is
   3316157 bytes, and its SHA-256 is the `.pdf` line of `MANIFEST.sha256`.
   Compare the two. If the address stops working, request a free copy at
...
The build needs `pdftotext` from poppler. If the command is absent, ask the user
to install poppler (`sudo pacman -S poppler` on Arch Linux), because the
installation needs a password. Do not extract poppler into a local prefix, and
do not substitute a different PDF tool: the parser reads the column layout that
`pdftotext -layout` makes, and the hashes in `MANIFEST.sha256` are those of
poppler 26.07.0.
...
The index has six columns:

1. The word.
2. The part of speech.
3. `Y` or `N` for approved.
4. The approved meaning or the approved alternatives.
5. The STE example.
6. The non-STE example.

Each row is one word plus one part of speech. When a word has more than one
meaning or alternative, a ` | ` separates them in column 4. The example
columns use the same separator. Segment N of an example column belongs to
segment N of column 4.

The introduction of the dictionary counts 875 approved and 1274 unapproved
words. The index has 879 approved and 1317 unapproved rows. The counts
differ because a word with two parts of speech makes two rows, and because
70 words are in the two categories (for example, GET is approved as "obtain"
and not approved as "become").

For the full entry in its page layout, grep the text:

```
grep -n -A12 '^<word> (v)' references/ASD-STE100_ISSUE9.txt
```

The index is derived from a fixed-width layout. If the two files disagree,
the text file is correct. Do not guess an alternative for a word that the
index does not list.
